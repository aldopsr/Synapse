...
class MaterialController extends Controller
{
    public function index()
    {
        $materials = Material::select('id', 'title', 'description', 'content', 'image', 'model_3d_path', 'has_ar', 'has_practice', 'created_at', 'user_id', 'course_id')
            ->with('user:id,name')->with('ar_assets')->with('questions')->latest()->get();
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->hasFile('image')) {
            $updateData['image'] = cloudinary()->upload($request->file('image')->getRealPath(), ['folder' => 'materials'])->getSecurePath();
        }
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'content' => 'required|string', 'image' => 'nullable|image|mimes:jpeg,png,jpg,webp|max:2048',
            'model_3d' => 'nullable|file|max:20480',
        ]);
        $imagePath = null; $model_3d_path = null; $has_ar = false;
        if ($request->hasFile('image')) {
            $imagePath = cloudinary()->upload($request->file('image')->getRealPath(), ['folder' => 'materials'])->getSecurePath();
        }
        if ($request->hasFile('model_3d')) {
            $model_3d_path = cloudinary()->uploadFile($request->file('model_3d')->getRealPath(), ['folder' => 'models', 'resource_type' => 'raw'])->getSecurePath();
            $has_ar = true;
        }
        $material = Material::create([
            'title' => $request->title, 'description' => $request->description,
            'content' => $request->content, 'user_id' => $request->user()->id,
            'image' => $imagePath, 'model_3d_path' => $model_3d_path,
            'has_ar' => $has_ar, 'visibility' => $request->input('visibility', 'mahasiswa'),
        ]);
    ...
    {
        $request->validate(['question' => 'required|string', 'options' => 'required|array|min:2', 'answer' => 'required|string']);
        $material = Material::find($material_id);
        if (!$material) return response()->json(['message' => 'Materi tidak ditemukan'], 404);
        $question = \App\Models\Question::create([
            'material_id' => $material_id, 'question' => $request->question,
            'options' => $request->options, 'answer' => $request->answer, 'explanation' => $request->explanation,
        ]);
    ...
    public function arGallery(Request $request)
    {
        ...
        if ($user && $user->role === 'dosen') $query->where('course_id', $user->course_id);
    ...
    public function getByCourse(Request $request, $course_id)
    {
        try {
            $user = $request->user();
            $query = Material::where('course_id', $course_id)->with('ar_assets')->with('questions');
            if (!$user || $user->role === 'public') $query->where('visibility', 'public');
    ...
    public function storeByCourse(Request $request, $course_id)
    {
        ...
        $has_ar = false; $model_3d_path = null; $imagePath = null;
        if ($request->hasFile('image')) {
            $imagePath = cloudinary()->upload($request->file('image')->getRealPath(), ['folder' => 'materials'])->getSecurePath();
        }
        if ($request->hasFile('model_3d')) {
        ...
        $material = Material::create([
            'title' => $request->title, 'description' => $request->description,
            'content' => $request->content, 'course_id' => $course_id,
